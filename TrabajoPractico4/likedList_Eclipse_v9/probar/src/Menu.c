...
int menu_principal(int* respuesta)
{
	...
	if(respuesta != NULL)
	{
		...
		puts("10. Salir.");
		puts("11. Probar Funciones que faltan.");
		if(!utn_pedirEntero(respuesta, "Ingrese una opción.", "Error.", 1, 11, 2, 1))
		{
			retorno = 0;
		}
		else
 ...
 * 					 ( -1 ) error.
 */
int menu_probar(int* opcion)
{
	int retorno = -1;
	if(opcion != NULL)
	{
		puts("1. Probar conteins all, sublist."
			"\n2._Probar clone.");
